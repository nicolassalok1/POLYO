...
def main() -> None:
    ...
    try:
        messages = load_with_fallback(Path(args.input).name)
        if not messages:
            logger.error("No input messages found (including fallback) for %s", args.input)
            ensure_empty(out_path)
            return

        preds = run_sentiment(messages, logger)
        dump_jsonl(out_path, preds)
        logger.info("Wrote %d sentiment rows to %s", len(preds), out_path)
    except Exception as exc:
        logger.error("ERROR: step02 failed (%s). Leaving log empty.", exc)
        ensure_empty(out_path)
